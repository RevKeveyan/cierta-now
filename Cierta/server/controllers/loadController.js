...
// const { putObject, getSignedGetUrl, deleteObject } = require("../utils/s3");
...
//       if (req.files.carrierImageFile) {
//         carrierImageFiles = req.files.carrierImageFile.map((file) => file.path);
//       }
//     }
...
//     if (loadData.costomerEmail.length > 0) {
//       try {
...
//       statusCode = 400;
//       response.validationErrors = Object.values(err.errors).map(
//         (e) => e.message
//       );
//     }
...
//     if (req.files) {
//       if (req.files.images) {
//         newVehicleImages = req.files.images.map((file) => file.path);
//       }

//       if (req.files.carrierImageFile) {
//         newCarrierImages = req.files.carrierImageFile.map((file) => file.path);
//       }
//     }

//     // Удаление помеченных изображений
//     removedVehicleImages.forEach((imagePath) => {
//       if (fs.existsSync(imagePath)) {
//         fs.unlinkSync(imagePath);
//       }
//     });

//     removedCarrierImages.forEach((imagePath) => {
//       if (fs.existsSync(imagePath)) {
//         fs.unlinkSync(imagePath);
//       }
//     });
...
//     updates.images = [
//       ...(oldLoad.images || []).filter(
//         (img) => !removedVehicleImages.includes(img)
//       ),
//       ...newVehicleImages,
//     ];
...
//           (img) => !removedCarrierImages.includes(img)
//         ),
//         ...newCarrierImages,
//       ],
//     };
...
//     // Отправка email при изменении статуса
...
// Вспомогалка: загрузить массив файлов в S3 и вернуть массив ключей
async function uploadFilesToS3(files = [], loadId, kind) {
  const keys = [];
  for (const file of files) {
    const Key = s3KeyForLoadImage(loadId, file.mimetype, new Date(), kind);
...
exports.updateLoad = async (req, res) => {
  try {
    ...
    for (const key of removedVehicleImages) {
      try { await deleteObject(key); } catch (e) { console.error("S3 delete vehicle err:", e); }
    }
    for (const key of removedCarrierImages) {
      try { await deleteObject(key); } catch (e) { console.error("S3 delete carrier err:", e); }
    }
...
exports.deleteLoad = async (req, res) => {
  try {
    ...
    for (const key of allKeys) {
      try { await deleteObject(key); } catch (e) { console.error("S3 delete err:", e); }
    }
